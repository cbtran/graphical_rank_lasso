...
#Graph generator. Adapted from huge package on CRAN. Modified according to the graph described in our paper.
                       ...
                       g = NULL, prob = NULL, vis = FALSE, verbose = TRUE) 
{
  ...
  rm(vis, verbose)
  gc()
  sim = list(sigma = sigma, 
             omega = omega, theta = theta, graph.type = graph)
  class(sim) = "sim"
  return(sim)
}
